...
### Measurement tools (in docs\dev\harness\, run from the repo root)
...
- Variant modules live under `docs\dev\harness\ab\` (`head` = pristine,
  `single_*` single-candidate, `*_internal` = pristine-HEAD logic + bracket). A pristine base
  `docs\dev\harness\module_head.psm1` is hash-checked (B70BC4E8) before regenerating candidates. Rebuild generators:
  `docs\dev\harness\mk_internal2.ps1` (A-suite logic-bracket variants), `docs\dev\harness\mk_prelude.ps1` (B-suite prelude-bracket
  variants), `docs\dev\harness\mk_corrected.ps1` (A1+A3 corrected set, logic + prelude brackets). Always re-verify 0 cross-contamination markers
...
powershell -NoProfile -File docs\dev\harness\e2e.ps1 -BudgetSeconds 300 -WarmSamples 40

# portion-3 module A/B (interleaved two-process, cold 3s idle):
Start-Process pwsh -ArgumentList @('-NoProfile','-File','.\docs\dev\harness\module_ab.ps1',`
...
```
- e2e.ps1 sets cwd to the repo root (parent of docs\dev\harness\), imports the module via
...
pwsh -NoProfile -File docs\dev\harness\mk_internal2.ps1
pwsh -NoProfile -File docs\dev\harness\mk_corrected.ps1
pwsh -NoProfile -File docs\dev\harness\mk_prelude.ps1
...
```
powershell -NoProfile -File .\docs\dev\harness\throttle_stats.ps1
```
