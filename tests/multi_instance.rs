...
impl MultiRepoHarness {
    ...
    fn render_a(&mut self) -> String {
        let p = self.a.path().to_path_buf();
        self.render_repo(&p)
    }
    fn render_b(&mut self) -> String {
        let p = self.b.path().to_path_buf();
        self.render_repo(&p)
    }
    fn write_a(&self, name: &str, content: &str) {
        self.a.write(name, content);
    }
    fn write_b(&self, name: &str, content: &str) {
        self.b.write(name, content);
    }
    fn remove_a(&self, name: &str) {
        self.a.remove(name);
    }
    fn git_a(&self, args: &[&str]) {
        self.a.git(args);
    }
}
...
fn version_isolation() {
    ...
    h.settle_and_poll();
    assert!(
        h.watcher.version(h.a.path()) > v_a_before,
        "A version should increase after write to A"
    );
    assert_eq!(
        h.watcher.version(h.b.path()),
        v_b_before,
        "B version unchanged"
    );
    ...
    h.settle_and_poll();
    assert!(
        h.watcher.version(h.b.path()) > v_b_before,
        "B version should increase after write to B"
    );
    assert_eq!(
        h.watcher.version(h.a.path()),
        v_a_mid,
        "A version unchanged"
    );
}
...
fn two_repos_git_add() {
    ...
    assert_ne!(
        a1, a2,
        "repo A should change after creating and staging file"
    );
    assert_eq!(b1, b2, "repo B should be unaffected");
}
...
fn two_repos_manual_rename() {
    ...
    std::fs::rename(
        h.a.path().join("rename_me.txt"),
        h.a.path().join("renamed.txt"),
    )
    .unwrap();
    h.settle_and_poll();
...
fn two_repos_stash_push_pop() {
    ...
    let b3 = h.render_b();
    assert_ne!(
        a_stashed, a_restored,
        "repo A should change after stash pop"
    );
    assert_ne!(
        a1, a_restored,
        "repo A should show modified after stash pop"
    );
...
fn two_repos_ignored_file() {
    ...
    assert_eq!(
        a_clean, a2,
        "repo A should not change after creating ignored file"
    );
    assert_eq!(b1, b2, "repo B should be unaffected");
}
...
fn two_repos_independent_ops_both() {
    ...
    assert_ne!(
        a2, a3,
        "repo A should change again after modifying tracked file"
    );
